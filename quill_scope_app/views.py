...
def new_topic(request):
    ...
    if request.method != 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('quill_scope_app:topics')
...
def new_entry(request, topic_id):
    ...
    if request.method != 'POST':
        ...
        if form.is_valid():
            ...
            new_entry.save()
            return redirect('quill_scope_app:topic', topic_id=topic_id)
...
def edit_entry(request, entry_id):
    ...
    if request.method != 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('quill_scope_app:topic', topic_id=topic.id)
        
    # Display a blank or invalid form
    context = {'entry':entry, 'topic':topic, 'form':form}
    return render(request, 'quill_scope_app/edit_entry.html', context)